...
GIGABYTE = (1024**3)
SLIDESEGMENTER_CHECKPOINT_PATH = r"spitz_classification/custom_slidesegmenter_parameters/checkpoint_I59000.tar"
SLIDESEGMENTER_SETTINGS_PATH = r"spitz_classification/custom_slidesegmenter_parameters/settings.json"
...
class PreprocessingService(BaseService):
    ...
    def __init__(
        ...
        self.segmenter = SlideSegmenter(
            ...
            model_folder = '2024-01-10',
            device = self.device,
        )
        self.segmenter._load_model(
            ModifiedUNet, 
            SLIDESEGMENTER_CHECKPOINT_PATH, 
            SLIDESEGMENTER_SETTINGS_PATH,
        )
    ...
    def _slide_preprocessing(self, slide_paths: list[Path], visualization_path: Path,
        ...
        uncorrected_tile_information = tessellate(
            ...
            pen_marking_map=pen_marking,
            max_pen_marking_fraction=self.settings['max_pen_marking_fraction'],
        ...
        if len(uncorrected_tile_information):
            for cross_section, tiles in uncorrected_tile_information.items():
                corrected_tiles = []
                for position, location, shape, pen_marking_fraction in tiles:
