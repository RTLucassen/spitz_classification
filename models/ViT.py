...
class EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        scale_init_value: Optional[float],
        dropout_prob: float,
        ...
        Args:
            ...
            mlp_ratio:  Ratio between hidden and input/output feature vector 
                length of MLP.
            scale_init_values:  Value for scale layer parameter initialization.
        ...
        # initialize first layer scale
        if scale_init_value:
            self.ls1 = LayerScale(embed_dim, init_values=scale_init_value)      
        else:
        ...
        # initialize second layer scale
        if scale_init_value:
            self.ls2 = LayerScale(embed_dim, init_values=scale_init_value)      
        else:
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        scale_init_value: Optional[float] = 1e-5,
        ...
        if self.patient_dim is not None and self.patient_token:
        ...
        if self.n_classes is None:
            ...
            self.classifier = nn.Linear(self.embed_dim + self.patient_dim, self.n_classes)
        else:
    ...
    def forward(
        ...
        return_last_feat_vector: bool = False,
        ...
        Args:
            ...
            return_last_feat_vector:  Indicates whether the last feature vector
                before the classification layer is returned.
        ...
        if return_last_feat_vector and return_last_self_attention:
            return pred, x, attn
        elif return_last_feat_vector:
            return pred, x
        elif return_last_self_attention:
            return pred, attn
        else:
            return pred
...
def convert_UNI_state_dict(
    path: Union[str, Path], 
    load_pos_embed: bool,
    pytorch_attn_imp: bool,
) -> dict:
    """
    Convert the state dict from the original UNI model to our implementation.
